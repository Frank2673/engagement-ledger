...
- **工程**
  ...
  - 240 个单元测试，含篡改检测、HMAC、表格注入、IPv6、委托归属、窗口合规、
    证据策略、授权书归档、**整链重写攻击实测**、CLI 端到端
  - `scripts/verify.mjs` 运营级校验 39 项（端到端闭环 / 篡改检测与归属 / 凭证守卫 /
    授权书登记闭环 / **整链重写与回滚的锚定实测** / 安全红线），本地与 CI 共用同一份代码
